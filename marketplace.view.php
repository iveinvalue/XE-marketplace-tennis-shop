...
class marketplaceView extends marketplace
{
	...
	function init()
	{
		...
		$oSecurity->encodeHTML('document_srl', 'comment_srl', 'vid', 'mid', 'page', 'category', 'search_target', 'search_keyword', 'sort_index', 'order_type', 'trackback_srl', 'p_f', 'p_t', 'h_f', 'h_t', 'b_f', 'b_t', 'w_f', 'w_t', 's_1', 's_2', 'f_1', 'f_2', 'f_3', 'l_f', 'l_t', 'g_s', 'list_count', 'priority_area', 'item_condition.', 'item_status');
	...
	function dispMarketplaceContentList(){
		...
		if(is_numeric(Context::get('p_f')))
			$args->p_f = Context::get('p_f');
		if(is_numeric(Context::get('p_t')))
			$args->p_t = Context::get('p_t');
		
		if(is_numeric(Context::get('h_f')))
			$args->h_f = Context::get('h_f');
		if(is_numeric(Context::get('h_t')))
			$args->h_t = Context::get('h_t');
		
		if(is_numeric(Context::get('b_f')))
			$args->b_f = Context::get('b_f');
		if(is_numeric(Context::get('b_t')))
			$args->b_t = Context::get('b_t');

		if(is_numeric(Context::get('w_f')))
			$args->w_f = Context::get('w_f');
		if(is_numeric(Context::get('w_t')))
			$args->w_t = Context::get('w_t');
		
		if(is_numeric(Context::get('s_1')))
			$args->s_1 = Context::get('s_1');
		if(is_numeric(Context::get('s_2')))
			$args->s_2 = Context::get('s_2');
		
		if(is_numeric(Context::get('f_1')))
			$args->f_1 = Context::get('f_1');
		if(is_numeric(Context::get('f_2')))
			$args->f_2 = Context::get('f_2');
		if(is_numeric(Context::get('f_3')))
			$args->f_3 = Context::get('f_3');

		if(is_numeric(Context::get('l_f')))
			$args->l_f = Context::get('l_f');
		if(is_numeric(Context::get('l_t')))
			$args->l_t = Context::get('l_t');
		
		if(is_numeric(Context::get('g_s')))
			$args->g_s = Context::get('g_s');
